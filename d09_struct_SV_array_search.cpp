...
int main() {
	...
	*/
	...
	//cho user nhap ten sinh vien muon kiem tra thong tin
	char tenTimKiem[30];
	fflush(stdin);
	printf("\n >> vui long nhap ten cua sinh vien muon tim:  ");
	gets(tenTimKiem);


	//vong lap tim va xuat ra thong tin chi tiet cua sv dc yeu cau
	int count=0;
	for(int i=0; i<n; i++) {
		if(strstr(ds[i].tenSV, tenTimKiem)) {
			printf(" %3d, %-20s, %7.2f \n", ds[i].maso, ds[i].tenSV, ds[i].diemKQ);
			count++;
		}
	} // ket thuc vong FOR
	...
	if(count == 0){
		printf("\n >> Ko tim thay sinh vien co ten [%s] ! \n", tenTimKiem);
	}
